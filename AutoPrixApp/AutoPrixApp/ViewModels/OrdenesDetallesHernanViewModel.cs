...
namespace AutoPrixApp.ViewModels
{
    ...
    public class OrdenesDetallesHernanViewModel : BaseViewModel
    {
        ...
        VehiculosClientes _vehiculosClientes = new VehiculosClientes();

        public Command BtnApruebaOrden { get; }
        public Command BtnFinalizaOrden { get; }
        public Command BtnIngresoOrden { get; }
        public Command BtnSalidaOrden { get; }
        #endregion

        public OrdenesDetallesHernanViewModel(OrdenTrabajoCab obj1) {
            try
            {
                ...
                MuestraBotones();
                BtnApruebaOrden = new Command(this.OnBtnApruebaOrden);
                BtnFinalizaOrden = new Command(this.OnBtnFinalizaOrden);
        ...
        public async void OnBtnApruebaOrden()
        {
            try
            {
                ...
                if (Conection)
                {
                    ...
                    //await Shell.Current.Navigation.PushAsync(new PopupAprobarOrdenView());
                    await PopupNavigation.Instance.PushAsync(new PopupAprobarOrdenView(_ordenTrabajoCab.IdOrdenTrabajoCab));
        ...
        public async void OnBtnFinalizaOrden()
        {
            try
            {
                ...
                if (Conection)
                {
                    UserDialogs.Instance.ShowLoading("Cargando...");
                    await Task.Delay(2000);
                    _vehiculosClientes = null;

                    GetVehiculoOrden(_ordenTrabajoCab.IdOrdenTrabajoCab.ToString());
                    Int64 idusu = Int64.Parse(Preferences.Get("IdUsuario", 0l).ToString());
                    //await PopupNavigation.Instance.PushAsync(new FinalizaOrdenViewModel(_vehiculosClientes));
                    while (_vehiculosClientes == null)
                    {
                        await Task.Delay(1000);
                    }
                    await Shell.Current.Navigation.PushAsync(new FinalizaOrdenPage(_vehiculosClientes, _ordenTrabajoCab.IdOrdenTrabajoCab));
        ...
        private async void GetVehiculoOrden(string orden)
        {
            
            if (orden == null || orden.Equals("")) { return; }
            try
            {
                var json = await Globales.GetApiAppRoute("VehiculosClientes/GetVehiculoOrden", orden);
                jsonResult jsonres = JsonConvert.DeserializeObject<jsonResult>(json.ToString());

                if (jsonres.MENSAJE == "Ok")
                {
                    foreach (var item in jsonres.RESULTADO) {
                        _vehiculosClientes = JsonConvert.DeserializeObject<VehiculosClientes>(item.ToString());
                    }
                }
                else
                {
